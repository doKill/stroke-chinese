...
function Stroke() {
    ...
    function DrawFromTop2Bottom(ctx, stroke) {//从上到下
        stroke.cur_pos > stroke.end ? stroke.timer = 0 : (ctx.putImageData(stroke.imgData, stroke.left, stroke.top), stroke.cur_pos += eval(speed), ctx.save(), ctx.beginPath(), ctx.rect(stroke.left, stroke.begin, stroke.right - stroke.left, stroke.cur_pos - stroke.begin), ctx.clip(), ctx.beginPath(), stroke.CalcOutline(ctx), ctx.fill(), ctx.restore(), stroke.timer = setTimeout(function() {
            DrawFromTop2Bottom(ctx, stroke)
        },
        20))
    }
    function DrawRight2Left(ctx, stroke) {//从右到左
        stroke.cur_pos < stroke.end ? stroke.timer = 0 : (ctx.putImageData(stroke.imgData, stroke.left, stroke.top), stroke.cur_pos -= eval(speed), ctx.save(), ctx.beginPath(), ctx.rect(stroke.cur_pos, stroke.top, stroke.begin - stroke.cur_pos, stroke.bottom - stroke.top), ctx.clip(), ctx.beginPath(), stroke.CalcOutline(ctx), ctx.fill(), ctx.restore(), stroke.timer = setTimeout(function() {
            DrawRight2Left(ctx, stroke)
        },
        20))
    }
    function DrawBottom2Top(ctx, stroke) {//从下到上
        stroke.cur_pos < stroke.end ? stroke.timer = 0 : (ctx.putImageData(stroke.imgData, stroke.left, stroke.top), stroke.cur_pos -= eval(speed), ctx.save(), ctx.beginPath(), ctx.rect(stroke.left, stroke.cur_pos, stroke.right - stroke.left, stroke.begin - stroke.cur_pos), ctx.clip(), ctx.beginPath(), stroke.CalcOutline(ctx), ctx.fill(), ctx.restore(), stroke.timer = setTimeout(function() {
            DrawBottom2Top(ctx, stroke)
        },
        20))
    }
    ...
    this.CalcOutline = function(ctx) {
        ...
        h = 1; h <= j; ++h) {
            ...
            t = a[h % j].y;
            if (a[h % j].p) {
                if (1 < h || 1 == h && a[0].p) c ? ctx.lineTo(e, f) : (ctx.moveTo(e, f), c = !0),
                ctx.lineTo(s, t);
                ctx.stroke();
                e = s;
                f = t;
                d = a[h % j]
            } else a[(h + 1) % j].p ? (d = this.DrawCurveLine(ctx, d, a[h % j], a[(h + 1) % j], c), c = !0, e = d.x, f = d.y, ++h) : (f = a[(h + 1) % j], e = (a[h % j].x + f.x) / 2, f = (a[h % j].y + f.y) / 2, d = this.DrawCurveLine(ctx, d, a[h % j], new u(e, f, 1), c), c = !0, e = d.x, f = d.y)
        }
    };
...
function CharWriter() {
    function drawStroke(ctx, writer, sorder) {
        if ( - 1 == writer.cur_pos || !writer.strokes[writer.cur_pos].IsTimerStarted()) {
            if (0 <= writer.cur_pos && !(writer.cur_pos < sorder.e.length && sorder.e[writer.cur_pos + 1] & 128) && transientColor.length && !writer.t) {
                writer.timer = setTimeout(function() {
                        writer.t = !0;
                        var idx = sorder.getIndexOfOrder(sorder.CountNumItLessThan128(writer.cur_pos));
                        if (0 <= idx) {
                            for (var i = idx; i <= writer.cur_pos; ++i) writer.strokes[i].D(ctx);
                            for (i = idx; i <= writer.cur_pos; ++i) ctx.save(),
                                ctx.beginPath(),
                                writer.strokes[i].CalcOutline(ctx),
                                idx = strokeColor,
  ...
  09 023 f5cc 2add 1 f5cb 2ad0 0
    ...
    this.drawBG = function(c) {
        ...
        c.beginPath();
        //c.strokeStyle="red";
        for (var d = 0; d < this.strokes.length; ++d) this.strokes[d].CalcOutline(c);
        //c.restore();
        c.fillStyle = "#cccccc";
        c.fill()
    };
